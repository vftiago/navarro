...
import { applyEffects } from "../utils";
    ...
    cardEffects: [
      {
        ...
        callback: (gameState) => {
          ...
          const stateAfterEffects = applyEffects(
            ...
            randomServerCard.cardEffects,
            TriggerMoment.ON_FETCH,
          );

          const isFile = randomServerCard.type === CardType.FILE;

          const isAgenda = randomServerCard.type === CardType.AGENDA;

          const newDiscard = isFile
            ? [...stateAfterEffects.player.discard, randomServerCard]
            : stateAfterEffects.player.discard;

          const newScore = isAgenda
            ? [...stateAfterEffects.player.score, randomServerCard]
            : stateAfterEffects.player.score;

          const newGameState = {
            ...stateAfterEffects,
            server: {
              ...stateAfterEffects.server,
              currentDeck: stateAfterEffects.server.currentDeck.filter(
